...
import { Filters, NEO_OFFERINGS, UTM_SOURCES, DEVICES, COUNTRIES } from './FilterBar'
...
export default function CompareSection({ baseFilters }: { baseFilters: Filters }) {
    ...
    try {
      await Promise.all(
        segmentConfig.values.map(async (val) => {
          const params = new URLSearchParams({ startDate: baseFilters.startDate, endDate: baseFilters.endDate })
  ...
  }, [segmentBy, segmentConfig.values, baseFilters.startDate, baseFilters.endDate])
  ...
  return (
    <div>
      <div style={{ fontSize: 16, fontWeight: 700, color: C.textHi, marginBottom: 16 }}>Segment Comparison</div>

      <div style={{ display: 'flex', gap: 12, alignItems: 'center', marginBottom: 20 }}>
        <span style={{ fontSize: 12, color: C.sub }}>Compare by:</span>
        <select value={segmentBy} onChange={e => setSegmentBy(e.target.value)} style={selectStyle}>
      ...
      {loading && <div style={{ color: C.cyan, padding: 12, fontSize: 13 }}>Fetching data for {segmentConfig.values.length} segments...</div>}
      ...
      {segments.length > 0 && !loading && (
        <>
          ...
          <div style={{ display: 'flex', gap: 16, flexWrap: 'wrap', marginBottom: 20 }}>
            <div style={{ background: C.card, borderRadius: 10, border: `1px solid ${C.border}`, padding: '20px', flex: '1 1 480px', minWidth: 0 }}>
              <div style={{ fontSize: 14, fontWeight: 700, color: C.textHi, marginBottom: 12 }}>
      ...
      {segments.length === 0 && !loading && (
        <div style={{ textAlign: 'center', padding: 30, color: C.sub, fontSize: 13, background: C.card, borderRadius: 10, border: `1px solid ${C.border}` }}>
